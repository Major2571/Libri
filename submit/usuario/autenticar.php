<?php
/*
 * Requisição para autenticação do cliente
 */
...
try {
	...
	if ( Util::verificarReferencia() && Util::verificarTokenCsrf($_POST['csrf-token'] ?? null) ) {
		...
		$resposta = Fachada::instancia('Lojista')->autenticar($_POST['email'] ?? null, $_POST['senha'] ?? null);

		//Definindo o retorno
		$retorno = [
			'tipo' => 'sucesso',
			'acao' => ($_SESSION['usuario']['perfil_completo'] ?? true) ? 'acessar-sistema' : 'completar-cadastro',
